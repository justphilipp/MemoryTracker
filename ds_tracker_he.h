...
#ifndef COMMON_SRC_H_DS_TRACKER_HE_H_
#define COMMON_SRC_H_DS_TRACKER_HE_H_
...
template<class T>
class HETracker : public BaseTracker<T> {
 private:
  int task_num;
  int he_num;
  int epochFreq;
 ...
 public:
  ...
  HETracker(int task_num, int he_num, int epochFreq, int emptyFreq, bool collect) :
      BaseTracker<T>(task_num), task_num(task_num), he_num(he_num), epochFreq(epochFreq), freq(emptyFreq),
      collect(collect) {
  ...
  T *read(std::atomic<T *> const &obj, int index, int tid) {
...
#endif  // COMMON_SRC_H_DS_TRACKER_HE_H_
