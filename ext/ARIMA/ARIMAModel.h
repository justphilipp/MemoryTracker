...
public:
  ...
  /** acquire p, q parameters set.
   * @param period: difference's distance(i.e data[i + period] - data[i])
   * @param notModel: already acquired parameters, which are best.
   * @param needNot: already have some parameters set obtained.
   *
   * */
  std::vector<int> getARIMAModel(int period, std::vector<std::vector<int>> notModel, bool needNot) {
    ...
    std::vector<int> bestModel(3);  // p, q, AIC
    int type = 0;
    ModelType mtype;
    ...
    for (int i = 0; i < cnt; ++i) {
      ...
      if (model[i][0] == 0) {
        ...
        //coe.assign(tmp.begin(),tmp.end());
        type = 3;
        mtype = ModelType::ARMA;
      }
      ARMAMath ar_math;
      double aic = ar_math.getModelAIC(coe, data, mtype);
...
//        }
//      }
      // TODO(algo): I'm missing something? data's index should be n - i - 1, since @param predict should be the next value!
      for (int i = 0; i < p; i++) {
        tmpAR += arCoe[i] * data[n - i - 2];
      }
//      assert(tmpAR == tmpAR2);
      predict = (int) (tmpAR);
    } else {
