...
#ifndef COMMON_SRC_H_DS_LOCKFREE_LINKEDLIST_BOA_TRACKER_H_
#define COMMON_SRC_H_DS_LOCKFREE_LINKEDLIST_BOA_TRACKER_H_
...
class KWDBLockFreeLinkedListBOATracker : public RetiredMonitorable {
 ...
 public:
  explicit KWDBLockFreeLinkedListBOATracker(int task_num, TrackerType tracker_type = Range_new) :
  ...
  KWDBLockFreeLinkedListBOATracker(const KWDBLockFreeLinkedListBOATracker &other) = delete;

  KWDBLockFreeLinkedListBOATracker(KWDBLockFreeLinkedListBOATracker &&other) = delete;

  KWDBLockFreeLinkedListBOATracker &operator=(const KWDBLockFreeLinkedListBOATracker &other) = delete;

  KWDBLockFreeLinkedListBOATracker &operator=(KWDBLockFreeLinkedListBOATracker &&other) = delete;

  ~KWDBLockFreeLinkedListBOATracker() {
...
template<typename T>
template<typename... Args>
bool KWDBLockFreeLinkedListBOATracker<T>::Emplace(int tid, Args &&... args) {
...
template<typename T>
bool KWDBLockFreeLinkedListBOATracker<T>::Delete(const T &data, int tid) {
  MarkPtr *prev = nullptr;
  Node *cur = nullptr;
  Node *next = nullptr;
...
template<typename T>
bool KWDBLockFreeLinkedListBOATracker<T>::Search(const T &data, MarkPtr **prev, Node **cur, Node **nxt, int tid) {
  while (true) {
    bool cmark = false;
    *prev = &head_->ui;
...
#endif  // COMMON_SRC_H_DS_LOCKFREE_LINKEDLIST_BOA_TRACKER_H_
